...
class OFDMSim:
    ...
    def __init__(self, K = 64, CP = 16, 
                    ...
                    controlCarriers2 = np.array([]),
                    numberOfGuardCarriers = 6, dataModulation = "QPSK", controlModulation = "BPSK", controlModulation2 = "BPSK",
                    #channelResponse = [1, 0, 0.3+0.3j],
                    channelResponse = [1],
                    SNR = 6, data = ['a', 'b', 'c', 'd', 'e'], payloadLength = 1920,
                    control = [1, 1, 0, 1, 0, 0, 0, 0, 1, 0, 1, 1], control2 = [1, 1, 0, 1, 0, 0, 0, 0, 1, 0, 1, 1],
                    collidingPayloadLength = 0, collidingGain = 0, dataCoding = "1/1", controlCoding = "1/2", controlCoding2 = "1/2"):
        ...
        self.controlCarriers2 = controlCarriers2

        # data carriers are all remaining carriers
        self.dataCarriers = np.setdiff1d(np.setdiff1d(np.setdiff1d(np.setdiff1d(self.allCarriers, self.pilotCarriers), self.controlCarriers), self.controlCarriers2), guardCarriers)
        ...
        self.controlMappingTable2 = self.stringModulationToMappingTable(controlModulation2)
        ...
        self.controlMu2 = int(np.log2(len(self.controlMappingTable2)))
        ...
        self.controlDemappingTable2 = {v : k for k, v in self.controlMappingTable2.items()}
        ...
        if controlCoding2 == "1/1":
            self.controlCodingPuncturingMatrix2 = None
        elif controlCoding == "1/2":
            self.controlCodingPuncturingMatrix2 = self.coding12PunctureMatrix
        elif controlCoding == "2/3":
            self.controlCodingPuncturingMatrix2 = self.coding23PunctureMatrix
        elif controlCoding == "3/4":
            self.controlCodingPuncturingMatrix2 = self.coding34PunctureMatrix
        elif controlCoding == "5/6":
            self.controlCodingPuncturingMatrix2 = self.coding56PunctureMatrix
        elif controlCoding == "7/8":
            self.controlCodingPuncturingMatrix2 = self.coding78PunctureMatrix
        else:
            raise ValueError("Invalid coding for control!")
        ...
        numberOfOFDMSymbolsForControl2 = int(np.ceil((self.computeLengthWithCoding(len(control2) + 4.0, self.controlCodingPuncturingMatrix2))/ (len(self.controlCarriers2) * self.controlMu2)))
        ...
        targetControlLengthWithCoding2 = numberOfOFDMSymbolsForData * len(self.controlCarriers2) * self.controlMu2
        targetControlLengthWithoutCoding2 = self.computeLengthWithoutCoding(targetControlLengthWithCoding2, self.controlCodingPuncturingMatrix2)
        ...
        neededPaddingBits2 = targetControlLengthWithoutCoding2 - (len(control2) + 4)
        neededPaddingReps2 = neededPaddingBits2 // len(control2)
        neededPaddingRemainer2 = neededPaddingBits2 - neededPaddingReps2 * len(control2)
        self.control2 = control2 + control2*neededPaddingReps2 + control[0:neededPaddingRemainer2]

        # Compute and append a CRC4
        self.control = np.hstack((self.control, self.crc(self.control, self.crc4Poly)))
        self.control2 = np.hstack((self.control2, self.crc(self.control2, self.crc4Poly)))

        # Code control bits, if needed
        self.convolutionalDecoderInit()
        self.uncodedControl = self.control
        self.uncodedControl2 = self.control2
        ...
        if self.controlCodingPuncturingMatrix2 is not None:
            self.control2 = self.convolutionalCode(self.control2, self.controlCodingPuncturingMatrix2)
    ...
    # as input.
    def generateOFDMSymbol(self, dataSymbols, controlSymbols, controlSymbols2):
        ...
        # Fill the control carriers2 (if it exists)
        if len(self.controlCarriers2) <> 0:
            symbol[self.controlCarriers2] = controlSymbols2
    ...
    def demultiplexOFDMSymbol(self, OFDMSymbol):
        if (len(self.controlCarriers) <> 0) and (len(self.controlCarriers2) <> 0):
            return (OFDMSymbol[self.dataCarriers], OFDMSymbol[self.controlCarriers], OFDMSymbol[self.controlCarriers2])
        else:
    ...
    def generateTXSignal(self, colliding=False):
        ...
        paddedControlCarrierSymbols = np.array([0+0j] * (len(dataCarrierSymbols) / len(self.dataCarriers)) * self.controlMu * len(self.controlCarriers))
        ...
        if colliding == True:
            ...
            # Control bits are also generated randomly for the colliding signal.
            controlBits2 = self.generateRandomPayload(len(self.control2))
        else:

            # Control bits are already computed in binary form (including CRC).
            controlBits2 = self.control2

        # Group bits according to the number of bits in each symbol of
        # the modulation used for control
        groupedControlBits2 = self.groupBitsIntoSymbols(controlBits2, self.controlMu2)

        # Translate control bits into a sequence of control symbols
        controlCarrierSymbols2 = self.mapBitsToSymbols(groupedControlBits2, self.controlMappingTable2)

        # Pad control symbols to match the number of OFDM symbols used by the data carrier symbols
        paddedControlCarrierSymbols2 = np.array([0+0j] * (len(dataCarrierSymbols) / len(self.dataCarriers)) * self.controlMu2 * len(self.controlCarriers2))
        if len(controlCarrierSymbols2) < len(paddedControlCarrierSymbols2):
            paddedControlCarrierSymbols2[0:len(controlCarrierSymbols2)] = controlCarrierSymbols2
        else:
            paddedControlCarrierSymbols2 = controlCarrierSymbols2[0:len(paddedControlCarrierSymbols2)]
        controlCarrierSymbols2 = paddedControlCarrierSymbols2
        ...
        while dataPointer < len(dataCarrierSymbols):
            nextOFDMSymbol = self.generateOFDMSymbol(dataCarrierSymbols[dataPointer:(dataPointer + len(self.dataCarriers))], [controlCarrierSymbols[controlPointer:(controlPointer + len(self.controlCarriers))]], [controlCarrierSymbols2[controlPointer2:(controlPointer2 + len(self.controlCarriers2))]])
            ...
            controlPointer2 = controlPointer2 + len(self.controlCarriers2)
    ...
    def interpretRXSignal(self, rxSignal, l):
        ...
        controlCarrierSymbols2 = np.array([])
        sumPilotsEVM = 0
        for i in range(0, len(OFDMSymbols)):
            ...
            (nextDataCarrierSymbols, nextControlCarrierSymbols, nextControlCarrierSymbols2) = self.demultiplexOFDMSymbol(equalizedSymbol)
            ...
            controlCarrierSymbols2 = np.hstack((controlCarrierSymbols2, nextControlCarrierSymbols2))
        ...
        receivedControlBits2 = self.demap(controlCarrierSymbols2, self.controlDemappingTable2)
        ...
        receivedControlBits2 = receivedControlBits2.reshape(-1)

        # Remove padding.
        receivedDataBits = receivedDataBits[0:l]

        return (receivedDataBits, receivedControlBits, receivedControlBits2, dataEVM, controlEVM, pilotEVM)
    ...
    def simulate(self):
        ...
        (receivedDataBits, receivedControlBits, receivedControlBits2, dataEVM, controlEVM, pilotEVM) = self.interpretRXSignal(rxSignal, len(dataBits))
        ...
        # Control might have been padded by the physical layer. Remove the padding.
        receivedControlBits2 = receivedControlBits2[0:len(self.control2)]

        # Check BER on coded control.
        ControlBER2 = np.sum(abs(receivedControlBits2-self.control2))/float(len(self.control2))
        results["controlBER2"] = ControlBER2

        # Now decode it and check the BER again.
        if self.controlCodingPuncturingMatrix2 is not None:
            decodedReceivedControBits2 = self.convolutionalDecode(receivedControlBits2, self.controlCodingPuncturingMatrix2, len(self.uncodedControl2))
            ControlBER2 = np.sum(abs(decodedReceivedControBits2-self.uncodedControl2))/float(len(self.uncodedControl2))
        results["decodedControlBER2"] = ControlBER2
        results["controlCRCFailure2"] = 0

        if ControlBER2 > 0:

            results["controlLoss2"] = 1
            # Check if CRC matches.
            receivedControlCRC2 = receivedControlBits2[-4:]
            computedCRC2 = self.crc(receivedControlBits2[0:-4].tolist(), self.crc4Poly)

            if np.sum(abs(receivedControlCRC2-computedCRC2)) == 0:
                #print "CRC4 failed to catch corruption on control!"
                results["controlCRCFailure2"] = 1
        else:

            results["controlLoss2"] = 0
...
parser.add_argument('--controlCarriers', type=int, help='List of carriers used for control information. Can be empty', default=[16], nargs='+')
parser.add_argument('--controlCarriers2', type=int, help='List of carriers used for control information. Can be empty', default=[48], nargs='+')
...
parser.add_argument('--controlModulation2', help='Modulation used for control carriers. Either BPSK, QPSK, 8PSK, 16QAM, 64QAM or 256QAM.', default='BPSK')
...
parser.add_argument('--controlCoding2', help='"1/1", "1/2", "2/3" or "3/4"', default='1/2')
...
print("var_controlModulation2     = %s" % (opts.controlModulation2))
...
print("var_controlCoding2         = %s" % (opts.controlCoding2))
...
print("# dataBER\tdecodedDataBER\tdataLoss\tdataCRCFailure\tcontrolBER\tdecodedControlBER\tcontrolLoss\tcontrolCRCFailure\tcontrolBER2\tdecodedControlBER2\tcontrolLoss2\tcontrolCRCFailure2\tdataEVM\tcontrolEVM\tpilotEVM")
#print("# dataBER\tdecodedDataBER\tdataLoss\tdataCRCFailure")
#print("# controlBER\tdecodedControlBER\tcontrolLoss\tcontrolCRCFailure")
#print("# controlBER2\tdecodedControlBER2\tcontrolLoss2\tcontrolCRCFailure2\tdataEVM\tcontrolEVM\tpilotEVM")
#print("# dataEVM\tcontrolEVM\tpilotEVM")
for i in range(0, opts.reps):
    if i % 20 == 0:
        sim = OFDMSim(controlCarriers = np.array(opts.controlCarriers), controlCarriers2 = np.array(opts.controlCarriers2),
                K=opts.K,
                SNR=opts.snr,
                ...
                controlModulation2=opts.controlModulation2,
                data=[],
                ...
                controlCoding2=opts.controlCoding2)
        gc.collect()

    results = sim.simulate()
    print(" %.8f\t%.8f\t%d\t\t%d\t\t%.8f\t%.8f\t\t%d\t\t%d\t\t%.8f\t%.8f\t\t%d\t\t%d\t\t%.8f\t\t%.8f\t\t%.8f" % (results["dataBER"],
        ...
        results["controlBER2"], results["decodedControlBER2"], results["controlLoss2"], results["controlCRCFailure2"],
        results["dataEVM"], results["controlEVM"], results["pilotEVM"]))
    #print(" %.8f\t%.8f\t%d\t\t%d" % (results["dataBER"], results["decodedDataBER"], results["dataLoss"], results["dataCRCFailure"]))
    #print(" %.8f\t%.8f\t%d\t\t%d" % (results["controlBER"], results["decodedControlBER"], results["controlLoss"], results["controlCRCFailure"]))
    #print(" %.8f\t%.8f\t%d\t\t%d" % (results["controlBER2"], results["decodedControlBER2"], results["controlLoss2"], results["controlCRCFailure2"]))
    #print(" %.8f\t%.8f\t%.8f" % (results["dataEVM"], results["controlEVM"], results["pilotEVM"]))
    
    if results["dataLoss"] == 1 and results["controlLoss"] == 1 and results["controlLoss2"] == 1:
        totaldc = totaldc + 1;

    totald = totald + results["dataLoss"]
    totalc1 = totalc1 + results["controlLoss"]
    totalc2 = totalc2 + results["controlLoss2"]
    if results["controlLoss"] == 1 and results["controlLoss2"] == 1:
        totalc = totalc + 1;

print("")
print("SNR: %d | Tx ent dados: %f | Tx ent controle: %f | Tx ent controle1: %f | Tx ent controle2: %f | Tx perda dados e cont: %f" %  (opts.snr, 100-(totald*100/(opts.reps)), 100-(totalc*100/(opts.reps)), 100-(totalc1*100/(opts.reps)), 100-(totalc2*100/(opts.reps)), (totaldc*100/(opts.reps))))
print("===")
print("")
print("")
    #if i % 10 == 9:
